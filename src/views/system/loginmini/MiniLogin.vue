<template>
  <div :class="prefixCls" class="login-background-img">
            ...
            <div class="aui-formBox">
              <div class="aui-formWell">
                <div class="aui-flex aui-form-nav investment_title">
                  ...
                  <!--                    >{{ t('sys.login.mobileSignInFormTitle') }}2-->
                  <!--                  </div>-->
                </div>
                ...
                <div class="aui-formButton">
                  ...
                  <!--                  </div>-->
                  <div class="aui-flex">
                    <a class="aui-linek-code aui-flex-box" @click="registerHandleClick">网点注册</a>
                  </div>
                  <div class="aui-flex">
                    <a class="aui-linek-code aui-flex-box"
                       @click="registerHandleUserClick">企业用户注册</a>
                  </div>
                </div>
                <div>
                  <div class="aui-forget" style="float: left;cursor: pointer">
                    <a @click="downloadApp">app下载</a>
                  </div>
                  <div class="aui-forget" style="float: right;cursor: pointer">
                    <a @click="downloadPc">pc客户端下载</a>
                  </div>
                </div>
              </div>
    ...
    <div v-show="type === 'registerUser'" :class="`${prefixCls}-form`">
      <MiniRegisterUser ref="registerUserRef" @go-back="goBack" @success="handleSuccess"/>
    </div>
...
import MiniRegisterUser from "@/views/system/loginmini/MiniRegisterUser.vue";
import {companyQueryByIdApi} from "@/api/sys/company";
import {StatusEnum, UserTypeEnum} from "@/enums/sysEnum";
import {message} from "ant-design-vue";
import {deviceExpire} from "@/views/equipment/list/TDeviceTrace.api";
...
async function accountLogin() {
  ...
  try {
    ...
    if (userInfo) {
      ...
      setTimeout(() => {
        checkCompany(userInfo.companyId)
        checkDevice(userInfo.userType)
      }, 1000)
    }
  } catch (error) {
    notification.error({
...
function checkCompany(companyId) {
  companyQueryByIdApi({id: companyId}).then(res => {
    if (res && res.status == StatusEnum.INVALID.value) {
      createConfirm({
        title: '警告',
        content: '当前企业还未审核',
        iconType: 'warning',
...
function checkDevice(userType) {
  if (userType == UserTypeEnum.CLIENT.value) {
    deviceExpire().then(res => {
      if (res && res > 0) {
        createConfirm({
          title: '警告',
          content: `${res}台设备售后到期日已不足30天`,
          iconType: 'warning',
          closable: true,
          footer: null,
        });
...
function registerHandleClick() {
  ...
  setTimeout(() => {
    ...
    registerRef.value.setData({invitationCode: invitationCode});
  }, 300);
}
 ...
 * 注册企业用户
 */
function registerHandleUserClick() {
  type.value = 'registerUser';
  setTimeout(() => {
    registerUserRef.value.initForm();
    const invitationCode = getUrlParam('invitationCode');
    registerUserRef.value.setData({invitationCode: invitationCode});
  }, 300);
}
...
function downloadApp() {
  downloadByUrl({url: glob.appUrl})
}

function downloadPc() {
  downloadByUrl({url: glob.pcUrl})
}
