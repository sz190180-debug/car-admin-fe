          ...
          <div class="aui-formWell">
            <div class="aui-step-box">
              ...
              <div style="text-align: center">
                <h3>{{ t('sys.login.resetLoginPassword') }}</h3>
              </div>
            ...
            <div class="aui-account aui-account-line aui-forgot">
              <a-form-item>
                <div class="aui-input-line">
                  <a-input type="text" :placeholder="t('sys.login.mobile')"
                           v-model:value="formData.mobile"/>
                </div>
              </a-form-item>
              <div class="aui-input-line">
                <a-form-item>
                  <a-input type="text" :placeholder="t('sys.login.smsCode')"
            ...
            <!--                  </a-form-item>-->
            <!--                </div>-->
            <!--                &lt;!&ndash; 重置密码 end &ndash;&gt;-->
            <!--              </a-form>-->
            <!--                &lt;!&ndash; 重置成功 begin &ndash;&gt;-->
            ...
            <!--                  <h3>恭喜您，重置密码成功！</h3>-->
            <!--                </div>-->
            <!--                &lt;!&ndash; 重置成功 end &ndash;&gt;-->
            <!--            </div>-->
            <div class="aui-formButton" style="padding-bottom: 40px">
              <div class="aui-flex" v-if="activeKey === 1 || activeKey === 2">
                <a class="aui-link-login aui-flex-box" @click="nextStepClick">重置</a>
              </div>
              <div class="aui-flex" v-else>
                <a class="aui-linek-code aui-flex-box"
                   @click="toLogin">{{ t('sys.login.goToLogin') }}</a>
              </div>
              <div class="aui-flex">
                <a class="aui-linek-code aui-flex-box" @click="goBack"> {{
...
async function finishedPwd() {
  ...
  const resultInfo = await smsChangePassword(
    toRaw({
      password: pwdFormData.password,
  ...
  if (resultInfo.success) {
    accountInfo.password = pwdFormData.password;
    message.success(resultInfo.message)
    goBack()
  } else {
    //错误提示
    createErrorModal({
...
async function getLoginCode() {
  ...
  const result = await getChangePassCaptcha({phone: formData.mobile}).catch((res) => {
