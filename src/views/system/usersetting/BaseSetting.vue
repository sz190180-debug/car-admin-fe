<template>
  <div class="account-padding" :class="[`${prefixCls}`]">
        ...
        <div class="account-right border-bottom">
          <div v-if="!isEdit">
            ...
            <a-tooltip content="编辑姓名">
              <Icon class="pointer font-size-17 gray-bd account-icon"
                    icon="ant-design:edit-outlined"
                    @click="editHandleClick"/>
            </a-tooltip>
          </div>
          <div v-else>
            <a-input ref="accountNameEdit" :maxlength="100" v-model:value="userInfo.realname"
                     @blur="editRealName"/>
          </div>
          <div class="use-day">
    ...
    <div class="account-data border-bottom">
      ...
      <div class="account-detail">
        <div class="font-size-15 font-bold font-color-gray" style="margin-bottom: 16px">详细资料
        </div>
        ...
        <div class="margin-bottom-10 nowarp font-size-13">
          <span class="gray-75 item-label">邀请码</span>
          <a class="gray-3"
             @click="openQrCode">{{ userInfo.invitationCode ? userInfo.invitationCode : "" }}</a>
        </div>
      ...
      <div class="account-info">
        <div class="font-size-15 font-bold font-color-gray" style="margin-bottom: 16px">联系信息
        </div>
        <div class="margin-bottom-10 font-size-13">
  ...
  <a-modal v-model:open="qrCodeVisible" :width="200">
    <template #closeIcon>

    </template>
    <template #footer>
      <div style="display: flex; justify-content: center; gap: 8px;">
        <a-button type="primary" @click="downloadQrCodeImg">下载图片</a-button>
        <a-button type="primary" @click="downloadQrCodePdf">下载PDF</a-button>
      </div>
    </template>
    <QrCode ref="qrRef" :value="qrCodeValue"/>
  </a-modal>
...
const qrCodeValue = ref(glob.webUrl + '/login?invitationCode=' + userStore.getUserInfo.invitationCode)
