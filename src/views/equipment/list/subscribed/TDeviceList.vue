<template>
  <div class="p-2">
        ...
        <a-row :gutter="24">
          ...
          <a-col :lg="6">
            <a-form-item name="mac">
              ...
              <a-input placeholder="请输入MAC地址" v-model:value="queryParam.mac"
                       allow-clear></a-input>
            </a-form-item>
          </a-col>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection">
      <!--插槽:table标题-->
      <template #tableTitle>
        <a-button type="primary" @click="handleInDevice"
                  preIcon="ant-design:plus-outlined"> 入库
        </a-button>
        <!--        <a-button type="primary"-->
        <!--                  preIcon="ant-design:export-outlined" @click="onExportXls"> 导出-->
        <!--        </a-button>-->
        <!--        <j-upload-button type="primary"-->
        <!--                         preIcon="ant-design:import-outlined" @click="onImportXls">导入-->
        <!--        </j-upload-button>-->
...
<!--        <template v-if="column.key == 'rcs'">-->
<!--          <a v-if="text" :href="getRcsPath(text)"-->
<!--             target="_blank"-->
...
<!--        </template>-->
        <template v-if="column.key == 'location'">
          <a v-if="record.longitude && record.latitude" @click="skipLocation(record)">
            {{ record.longitude }},
            {{ record.latitude }}
          </a>
        </template>
    ...
    <InputModal ref="inDeviceModal" title="入库" label="设备序列号"
                @submit="inDeviceSubmit"/>
  </div>
</template>
...
import {
  ...
  returnAgree,
  returnDevice,
} from '/@/views/equipment/list/TDeviceTrace.api';
...
import {ActiveEnum, EnableEnum, OutStatusEnum, RunEnum} from "@/enums/sysEnum";
import {message} from "ant-design-vue";
import InputModal from "@/components/Modal/InputModal.vue";
import {inStore, sold} from "@/views/equipment/list/TDevice.api";
...
const inDeviceModal = ref()

/**
 * 设备出库
 */
function handleInDevice() {
  inDeviceModal.value.open()
}

function inDeviceSubmit(text) {
  inStore({seq: text}).then(res => {
    if (res.success) {
 ...
 * 同意申请退货
 */
function handleAgreeReturn(record: Recordable) {
  returnAgree({id: record.id}).then(res => {
    if (res.success) {
      message.success(res.message)
    } else {
      message.error(res.message)
 ...
 * 申请退货
 */
function handleReturnDevice(record: Recordable) {
  returnDevice({id: record.id}).then(res => {
    if (res.success) {
      message.success(res.message)
    } else {
      message.error(res.message)
  ...
  return [
    {
      ...
      ifShow: record.outStatus == OutStatusEnum.NOTDELIVERY.value
    },
  ];
}
...
function getDropDownAction(record) {
  return [
    ...
    },
    {
      ...
      popConfirm: {
        title: '是否确认申请退货',
        confirm: handleReturnDevice.bind(null, record),
        placement: 'topLeft',
      },
      ifShow: record.outStatus == OutStatusEnum.NOTDELIVERY.value
    },
