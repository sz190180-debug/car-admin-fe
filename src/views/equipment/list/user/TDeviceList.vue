<template>
  <div class="p-2">
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection">
      <!--插槽:table标题-->
      <template #tableTitle>
        <a-button type="primary" @click="handleActivate"
                  preIcon="ant-design:plus-outlined"> 激活设备
        </a-button>
    ...
    <InputModal ref="activateModal" title="激活" label="设备序列号"
                @submit="activateSubmit"/>
  </div>
</template>
...
import {ActiveEnum, EnableEnum, ReturnStatusEnum, RunEnum} from "@/enums/sysEnum";
import {message} from "ant-design-vue";
import TDeviceTranceModal from "@/views/equipment/list/my/components/TDeviceTranceModal.vue";
import {list} from "@/views/equipment/list/TDeviceClient.api";
import InputModal from "@/components/Modal/InputModal.vue";
import {returnDevice} from "@/views/equipment/list/TDeviceTrace.api";
...
function handleActivate() {
  activateModal.value.open()
}

function activateSubmit(text) {
  active({seq: text}).then(res => {
    if (res.success) {
      message.success(res.message)
    } else {
      message.error(res.message)
  ...
  return [
    {
      ...
      ifShow: record.returnStatus == ReturnStatusEnum.NORETURN.value,
    },
  ];
}
