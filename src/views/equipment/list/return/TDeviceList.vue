...
import {ActiveEnum, ReturnStatusEnum, RunEnum, UserTypeEnum} from "@/enums/sysEnum";
import {message} from "ant-design-vue";
import {returnAgree, returnList, returnRefuse} from "@/views/equipment/list/TDeviceTrace.api";
...
const filterColumns = computed(() => {
  if (userStore.getUserInfo.userType == UserTypeEnum.SUPER.value || userStore.getUserInfo.userType == UserTypeEnum.COMPANY.value) {
    columns.push(
      {
        title: '退货服务网点名',
        align: "center",
        dataIndex: 'returnCompanyName',
      },
    )
  }
  if (userStore.getUserInfo.userType == UserTypeEnum.AGENT.value) {
    columns.push(
      {
        title: '退货客户名',
        align: "center",
 ...
 * 拒绝申请退货
 */
function handleRefuseReturn(record: Recordable) {
  returnRefuse({id: record.id}).then(res => {
    if (res.success) {
      message.success(res.message)
    } else {
      message.error(res.message)
  ...
  return [
    {
      ...
      popConfirm: {
        title: '是否确认同意退货',
        confirm: handleAgreeReturn.bind(null, record),
        placement: 'topLeft',
      },
      ifShow: record.returnStatus == ReturnStatusEnum.RETURNIN.value,
    },
    {
      label: '拒绝',
      popConfirm: {
        title: '是否确认拒绝退货',
        confirm: handleRefuseReturn.bind(null, record),
        placement: 'topLeft',
      },
      ifShow: record.returnStatus == ReturnStatusEnum.RETURNIN.value,
    },
