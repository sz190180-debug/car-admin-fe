...
<script lang="ts" setup>
  ...
  import { saveOrUpdateUser, getUserRoles, getUserDepartList, getAllRolesListNoByTenant, getAllRolesList } from '@/views/employee/user.api';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
   ...
   /*   if (data.record.relTenantIds && !Array.isArray(data.record.relTenantIds)) {
    ...
    //部门管理，新增用户，在岗位下添加人员的时候默认当前岗位为岗位
    updateSchema([
      {
        field: 'mainDepPostId',
  ...
  const getTitle = computed(() => {
    ...
    if (!unref(isUpdate)) {
      ...
      return unref(showFooter) ? '编辑员工' : '员工详情';
    }
