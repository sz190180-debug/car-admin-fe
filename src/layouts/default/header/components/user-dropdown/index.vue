<template>
  <Dropdown placement="bottomLeft" :overlayClassName="`${prefixCls}-dropdown-overlay`">
    ...
    <template #overlay>
      <Menu @click="handleMenuClick">
        <MenuItem itemKey="doc" :text="t('layout.header.dropdownItemDoc')"
                  icon="ion:document-text-outline" v-if="getShowDoc"/>
        <MenuDivider v-if="getShowDoc"/>
        <MenuItem itemKey="account" :text="t('layout.header.dropdownItemSwitchAccount')"
                  icon="ant-design:setting-outlined"/>
        <MenuItem itemKey="basicinfo"
                  v-if="getUserInfo.userType == UserTypeEnum.AGENT.value ||
        getUserInfo.userType == UserTypeEnum.CLIENT.value"
                  :text="t('layout.header.dropdownItemCompleteInfo')"
                  icon="ant-design:form-outlined"/>
        <MenuItem itemKey="password" :text="t('layout.header.dropdownItemSwitchPassword')"
                  icon="ant-design:edit-outlined"/>
        <!--        <MenuItem itemKey="depart" :text="t('layout.header.dropdownItemSwitchDepart')" icon="ant-design:cluster-outlined" />-->
        <MenuItem itemKey="cache" :text="t('layout.header.dropdownItemRefreshCache')"
        ...
        />-->
        <MenuItem itemKey="logout" :text="t('layout.header.dropdownItemLoginOut')"
  ...
  <EditBasicInfo v-if="basicInfoVisible" ref="editBasicInfoRef"/>
</template>
<script lang="ts">
// components
...
export default defineComponent({
  ...
  components: {
    ...
    EditBasicInfo: createAsyncComponent(() => import('./EditBasicInfo.vue')),
  },
  props: {
  ...
  setup() {
    ...
    const getUserInfo = computed(() => {
      const {realname = '', avatar, desc, userType} = userStore.getUserInfo || {};
      return {realname, avatar: avatar || headerImg, desc, userType};
    });
    ...
    async function editBasicInfo() {
      ...
      await getRefPromise(editBasicInfoRef);
      editBasicInfoRef.value.show(userStore.getUserInfo.companyId);
    }
